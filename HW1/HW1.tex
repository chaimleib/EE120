...
\begin{enumerate}
\item[a.] $h(t) = e^tu(t)$ is not \emph{BIBO} stable system, an example of a bounded input 
...
\item[b.] $h(t) = (t - 1)^2e^{1-t}u(t)$ The system is \emph{BIBO} stable since:
\begin{align*}
	y(t) = h(t) * x(t) = 
...
\item[c.] $h[n] = u[n - 4]$ is not \emph{BIBO} stable system, an example of a bounded input 
resulting in an unbound output is $x(t) = u(t)$ which results in the output:
\begin{align*}
y[t] = h[t] * x[t] = 
...
\item[d.] $Cos[2\pi n] u[n]$ is not \emph{BIBO} stable system, an example of a bounded input 
...
\item[e.] $\sum_{n = -\infty}^{\infty}\delta(t - 2n)$ is not \emph{BIBO} stable system, an example of a bounded input 
resulting in an unbound output is $x(t)= u(t)$ which results in the output
\begin{align*}
	y(t) = h(t) * x(t) = 
